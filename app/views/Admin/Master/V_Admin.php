                ...
                <div class="card mb-5 mb-xl-8">
                    <div class="d-flex flex-stack flex-wrap ms-10 mt-10">
                        ...
                        <div class="page-title d-flex flex-column align-items-start">
                            <!--begin::Title-->
                            <h1 class="d-flex text-dark fw-bold m-0 fs-3">Master Data Admin</h1>
                            <!--end::Title-->
                            <!--begin::Breadcrumb-->
                    ...
                    <div class="card-header border-0 pt-5">
                        <div class="d-flex align-items-center position-relative me-3 search_mekanik w-300px">
                            ...
                            <input type="text" name="search" value="<?= $search; ?>" class="form-control form-control-solid w-250px ps-13" aria-label="Cari admin" aria-describedby="button-cari-admin" placeholder="Cari admin" autocomplete="off">
                            <button type="button" onclick="search(false)" class="btn btn-primary d-none" type="button" id="button-cari-admin">
                        ...
                        <div class="card-toolbar">
                            ...
                            <div class="d-none justify-content-end" id="sistem_drag">
                                <button type="button" id="btn_hapus" onclick="submit_form(this,'#reload_table',0,'/deleted/admin/<?= base64url_encode('master/admin') ?>',true,true)" data-message="Apakah anda yakin akan menghapus data admin? data yang di hapus tidak akan bisa di kembalikan" class="btn btn-sm btn-light-danger me-3">Hapus</button>
                            </div>
                            ...
                            <!--end::Toolbar-->
                            <!--begin::Add admin-->
                             <button type="button" class="btn btn-sm btn-light" onclick="tambah_data()" data-bs-toggle="modal" data-bs-target="#kt_modal_admin">
                    ...
                    <div class="card-body py-3" id="base_table">
                        ...
                        <form action="<?= base_url('pengaturan/drag',true) ?>" method="POST" class="table-responsive" id="reload_table">
                            ...
                            <table class="table table-row-dashed table-row-gray-300 align-middle gs-0 gy-4">
                                ...
                                <tbody>
                                    ...
                                    <?php if ($result) : ?>
                                        <?php foreach ($result as $row) : ?>
                                            <tr>
                                                <td>
                                                    <div class="form-check form-check-sm form-check-custom form-check-solid">
                                                        <input class="form-check-input widget-9-check cursor-pointer child_checkbox" name="id_batch[]" onchange="child_checked()" type="checkbox" value="<?= $row->id_admin ?>">
                                                    </div>
                                                </td>
                                                <td>
                                                    <div class="d-flex align-items-center">
                                                        <div class="symbol symbol-45px me-5">
                                                            <img onclick="preview_image(this)" src="<?= image_check($row->foto, 'admin','user') ?>" alt="" class="cursor-pointer">
                                                        </div>
                                                ...
                                                <td>
                                                    <div class="d-flex justify-content-center align-items-center">
                                                        <div class="form-check form-switch">
                                                            <input onchange="switching(this,event,<?= $row->id_admin ?>)" data-url="<?= base_url('pengaturan/switch/admin',true) ?>" class="form-check-input cursor-pointer focus-info" type="checkbox" role="switch" id="switch-<?= $row->id_admin ?>" <?= ($row->status == 'Y') ? 'checked': ''; ?>>
                                                        </div>
                                                    </div>
                                                ...
                                                <td>
                                                    <div class="d-flex justify-content-end flex-shrink-0">
                                                        <button type="button" class="btn btn-icon btn-bg-light btn-active-color-info btn-sm me-1" title="Ubah data admin" onclick="ubah_data(this,<?= $row->id_admin ?>)" data-image="<?= image_check($row->foto,'admin','user'); ?>" data-bs-toggle="modal" data-bs-target="#kt_modal_admin">
                                                            <i class="ki-outline ki-pencil fs-2"></i>
                                                        </button>
                                                        <button type="button" onclick="hapus_data(event,<?= $row->id_admin; ?>,'master/hapus_admin','<b><?= $row->nama; ?></b>')" title="Hapus data admin" class="btn btn-icon btn-bg-light btn-active-color-danger btn-sm">
                                    ...
                                    <?php else : ?>
                                        <tr>
                                            <td colspan="5">
                                                <center>Data admin tidak ditemukan</center>
                                            </td>
                                        </tr>
                                    <?php endif; ?>
                                </tbody>
                            ...
                            <!--end::Table-->
                            <?= pagination(base_url('master/admin',true),$jumlah,$limit,$offset); ?>
                        </form>
                        <!--end::Table container-->
                    </div>
    ...
    <div class="modal-dialog modal-dialog-centered mw-650px">
        <div class="modal-content">
            <div class="modal-header">
                <h1 class="modal-title fs-5" id="title_modal">Tambah Admin</h1>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body mx-5 mx-xl-15 my-7">
                <!--begin::Form-->
                <form id="form_admin" class="form" action="<?= base_url('master/tambah_admin',true) ?>" method="POST" enctype="multipart/form-data">
                    <!--begin::Scroll-->
                    ...
                    <div class="text-center pt-15">
                        <button type="button" id="submit_admin" onclick="submit_form(this,'#form_admin',1)" class="btn btn-primary">
